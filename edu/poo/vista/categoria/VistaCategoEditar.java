...
public class VistaCategoEditar extends SubScene {
    ...
    public VistaCategoEditar(Stage ventanaPadre, BorderPane princ, Pane pane, double ancho,
            double alto, Categoria objCateExterna, int posicionArchivo) {
    ...
    private void crearTitulo() {
        ...
        Text titulo = new Text("Formulario actualización de categorías");
    ...
    private void crearFormulario() {
        ...
        cajaNombre.setText(objCategoria.getNombreCategoria());
        ...
        cajaImagen.setText(objCategoria.getNombreImagenPublicoCategoria());
        ...
        // Fila previsualización de la imagen
        fila = 6;
        imgPorDefecto = Icono.obtenerFotosExternas(objCategoria.getNombreImagenPrivadoCategoria(), 150);
        GridPane.setHalignment(imgPorDefecto, HPos.CENTER);
        miGrilla.add(imgPorDefecto, segundaColumna, fila);
        // *********************************************************************

        // Fila del botón actualizar (nos volamos la 7)
        fila = 8;
        Button btnEditar = new Button("Actualizar categoria");
        btnEditar.setPrefHeight(altoCaja);
        btnEditar.setMaxWidth(Double.MAX_VALUE);

        btnEditar.setTextFill(Color.web("#6C3483"));
        btnEditar.setFont(Font.font("Verdana", FontWeight.BOLD, tamanioFuente));
        btnEditar.setOnAction((e) -> {
            if (formularioDiligenciado()) {
                int codCat = objCategoria.getCodCategoria();
                ...
                String nocu = objCategoria.getNombreImagenPrivadoCategoria();

                Categoria nuevoCate = new Categoria(codCat, nomCat, estCat, 0, imaCat, nocu);

                if (ControladorCategoEditar.actualizar(posicion, nuevoCate, rutaImagenSeleccionada)) {
                    Mensaje.modal(Alert.AlertType.INFORMATION, null, "EXITO", "Categoría actualizada correctamente");
                } else {
                    Mensaje.modal(Alert.AlertType.ERROR, null, "ERROR", "Fatal error actualizando la categoría");
                }
            }
        });
        miGrilla.add(btnEditar, segundaColumna, fila);

        // Fila del botón regresar
        fila = 9;
        Button btnRegresar = new Button("Regresar");
        btnRegresar.setPrefHeight(altoCaja);
        ...
        btnRegresar.setOnAction((ActionEvent e) -> {
            panelCuerpo = ControladorCategoVentana.administrar(
