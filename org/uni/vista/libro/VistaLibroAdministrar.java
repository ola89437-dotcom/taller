...
import org.uni.controlador.LibroControladorEliminar;
...
public class VistaLibroAdministrar extends StackPane {
    ...
    public VistaLibroAdministrar(Stage ventanaPadre, double ancho, double alto) {
    ...
    private void crearTitulo() {
        ...
        titulo = new Text("ADMINISTRAR LIBROS (" + cant + ")");
        titulo.setFill(Color.web(Configuracion.AZUL_CLARO));
    ...
    private TableColumn<LibroDto, String> crearColumnaEditorial() {
        ...
        columna.setCellValueFactory(cellData -> {
            EditorialDto editorial = cellData.getValue().getIdEditorial();
            if (editorial != null) {
    ...
    private TableColumn<LibroDto, String> crearColumnaAutor() {
        ...
        columna.setCellValueFactory(cellData -> {
            AutorDto autor = cellData.getValue().getIdAutor();
            if (autor != null) {
        ...
        btnEliminar.setOnAction((e) -> {
            if (miTabla.getSelectionModel().getSelectedItem() == null) {
                Mensaje.mostrar(Alert.AlertType.WARNING, miEscenario,
                        "Atención", "Por favor selecciona un libro");
            } else {
                LibroDto objLibrito = miTabla.getSelectionModel().getSelectedItem();

                String mensaje = "¿Estás seguro?\n\n"
                        + "Código: " + objLibrito.getIdLibro() + "\n"
                        + "Libro: " + objLibrito.getNombreLibro() + "\n"
                        + "Precio: $" + String.format("%.2f", objLibrito.getPrecioLibro()) + "\n"
                        + "Año: " + objLibrito.getAnioLibro() + "\n\n"
                ...
                if (msg.showAndWait().get() == ButtonType.OK) {
                    ...
                    if (LibroControladorEliminar.borrar(posi)) {
                        int canti = LibroControladorListar.obtenerCantidadLibros();
                        titulo.setText("ADMINISTRAR LIBROS (" + canti + ")");

                        List<LibroDto> arrLibros = LibroControladorListar.obtenerLibro();
                        ObservableList<LibroDto> datosTablo = FXCollections.observableArrayList(arrLibros);
